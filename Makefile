...
# help:
# help: game-collecting Makefile help
# help:
...
venv:
	@rm -Rf venv
	@python3 -m venv venv --prompt game_collecting
...
.PHONY: check-lint
check-lint:
	@pylint --rcfile=.pylintrc game_collecting ./tests setup.py ./examples
...
.PHONY: docs
docs: coverage
	@cd docs; rm -rf source/api/game_collecting*.rst source/api/modules.rst build/*
	@cd docs; make html
...
.PHONY: dist-test
dist-test: dist
	@cd dist && ../tests/test-dist.bash ./game_collecting-*-py3-none-any.whl


# help: dist-upload                    - upload a wheel distribution package
.PHONY: dist-upload
dist-upload:
	@twine upload dist/game_collecting-*-py3-none-any.whl
