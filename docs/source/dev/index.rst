...
The game-collecting project implements a regression
...
not broken existing functionality. The game-collecting
...
.. code-block:: console
    ...
    (game_collecting) $ make test

To run tests verbosely use:

.. code-block:: console

    (game_collecting) $ make test-verbose
...
``game_collecting`` package has been installed. If this is
not the case then you will likely need to set the ``PYTHONPATH`` environment
variable so that the ``game_collecting`` package can be found.

.. code-block:: console

    (game_collecting) $ cd tests
    (game_collecting) $ python -m unittest

Individual unit tests can be run also.

.. code-block:: console

    (game_collecting) $ python -m test_version
...
.. code-block:: console
    ...
    (game_collecting) $ make sort-imports
...
.. code-block:: console
    ...
    (game_collecting) $ make check-static-analysis
...
.. code-block:: console
    ...
    (game_collecting) $ make check-lint
...
.. code-block:: console
    ...
    (game_collecting) $ make check-types
...
.. code-block:: console
    ...
    (game_collecting) $ make check-docs
...
.. code-block:: console
    ...
    (game_collecting) $ make serve-docs
...
- Create and push a repo tag to Github. As a convention use the package
  ...
  - This will trigger Github to create a release at:
    ...
    ::
        ...
        https://github.com/{username}/game_collecting/releases/{tag}
...
- Test the release distribution. This involves creating a virtual environment,
  ...
  .. code-block:: console
      ...
      (game_collecting) $ make dist-test

- Upload the release to PyPI using

  .. code-block:: console

      (game_collecting) $ make dist-upload

  The package should now be available at https://pypi.org/project/game_collecting/
